...
export const ProductEditScreen = ({ navigation }) => {
  ...
  const orderEditProduct = useSelector((state) => state.order.orderProduct);
  ...
  useEffect(() => {
    ...
    .then((res) => {
      ...
      if(orderEditProduct && orderEditProduct.pid == res.data.product.pid) {
        setQuantity(orderEditProduct.quantity);
        setSku(orderEditProduct.option_sku);
        let products_extra_arraya = orderEditProduct.extras;
       
        if (orderEditProduct.extras) {
          for (var key in orderEditProduct.extras) {
            if (orderEditProduct.extras.hasOwnProperty(key)) {
              let optionArray = [];
              for (var optionkey in orderEditProduct.extras[key].options) {
                let product_option_temp = optionkey;
                
                optionArray.push(product_option_temp);       
                 
              }
              products_extra_arraya[key] = optionArray;
            }
          }
        }
        console.log("*******Edit product addons testing********",products_extra_arraya);
        chooseExtras(products_extra_arraya);
        if(orderEditProduct.instructions != '')
        {
          setInstruction(orderEditProduct.instructions);
          setInstructionLink('- Remove Special Instructions');
        }  
  ...
  const addToCart = useCallback(
    (pid, optionSku, optionQuantity, extraOptions,instructions,price) => {
      ...
      // } else {
        ...
        var requestUrl = "/order/update_product";
        if(orderEditProduct && orderEditProduct.pid == pid) {
          formData.append('order_product_id', orderEditProduct.opid);
          requestUrl = "/order/update_product";
        }
          fetchAPI(requestUrl, {
            method: 'POST',
            headers: headers,
  ...
  const extraOptions = useMemo(() => {
    if (extrasChosen) {
      console.log("choosed choosen*****************",extrasChosen);
      let extra_options = [];
      for (var key in extrasChosen) {
        ...
        console.log("choosed extras*****************",extra_options);
      }
            ...
            <TouchableOpacity
              ...
              <View style={{flexDirection:'row'}}>
                ...
                <Text style={{color:'#FFFFFFA5',fontSize:17,marginLeft:10}}>
                 ...
                 <Text  style={{color:'#FFFFFF',fontSize:20,marginLeft:10,fontWeight: 'bold'}} >Update </Text>
        ...
        <ScrollView style={styles.container} alwaysBounceVertical={false}>
          ...
          <View style={styles.content}>
            ...
            {product.is_subscription == true && <AppText
              ...
              numberOfLines={2}>
              Renews on the 1st of { <AppText style={{ textTransform: 'lowercase'}}>{product.subscription_type_name_every_full}</AppText>}. Cancel any time.
            </AppText>}
...
ProductEditScreen.navigationOptions = ({ navigation }) =>
